...
Agents use the `remote-tab` npm package (Node.js 20 or newer). **The CLI is
preferred**; the same package also runs an MCP server. The agent skill has the
consent rules and the exact sequence: `npx -y remote-tab skill`. For wire
requests, see [Agent API](agent-api.md). For copyable host configuration, see
[examples](../examples/).

## CLI (preferred)

Run `npx -y remote-tab --help` for the command list, and
`npx -y remote-tab skill` for the agent skill. Create uses the same server URL
and API key environment variables as the MCP server. Later commands use the private
connection state saved by create, without retaining the platform API key.

```sh
rt() { npx -y remote-tab "$@"; }
STATE="$HOME/.local/state/remote-tab/example.json"
rt create --state "$STATE" --ttl 1800
# Deliver the returned code privately, then wait for the human to Share.
rt wait-ready --state "$STATE"
rt browser_snapshot '{}' --state "$STATE"
# Replace e1 with a ref from the latest snapshot; only click for an agreed task.
rt browser_click '{"ref":"e1"}' --state "$STATE"
rt handoff '{"message":"Please finish sign-in and click Done."}' --state "$STATE"
rt stop --state "$STATE"
rt ledger export --state "$STATE" --out ./session-ledger
```
...
Launch the stdio server with `npx -y -p remote-tab remote-tab-mcp`. From a
source checkout, `bun packages/mcp/src/main.ts` runs the same server. Set
...
The TypeScript library is in this repository (`packages/client`); it is not
published to npm.
